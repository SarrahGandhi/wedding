-- Arrival times are local wall times in IST. Existing dates retain unknown times.
...
begin
    ...
    for entry in select value from jsonb_array_elements(plans) loop
        ...
        foreach field in array array['guests', 'arrival_date', 'arrival_time', 'travel_mode', 'travel_details', 'pickup_by'] loop
